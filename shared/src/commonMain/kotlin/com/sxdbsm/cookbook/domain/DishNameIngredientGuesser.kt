...
object DishNameIngredientGuesser {
    ...
    private val STOP_WORDS = setOf(
        ...
        // [AI修改 2026-07-18] 打磨补充常见双字做法/风味词(醋溜排骨→醋溜误当食材等)
        "醋溜", "干烧", "酱烧", "蜜汁", "铁板", "藤椒", "椒麻", "怪味", "软炸", "干炸",
        "油炸", "香煎", "石锅", "砂锅",
    )

    /**
     * 做法/连接字：**库外候选**若以这些字**开头**，视为"做法+食材"的脏切分(烧鸡/包鸡/炒饭)而非干净食材名，拒绝提出。[AI生成 2026-07-18]
     *
     * 只拒**开头**命中的：叉烧/面包(做法字在词尾)仍保留为合法库外候选。宁缺勿误——错切分自动建库比漏一个更糟。
     */
    ...
    fun guessDetailed(
        ...
        while (i < name.length) {
            if (!used[i] && name[i].isCjk()) {
                ...
                // [AI修改] 打磨：库外候选若以做法/连接字开头(烧鸡/包鸡/炒饭)=脏切分，拒绝提出(防误当食材自动建库)。
                if (seg.length >= 2 && seg.first() !in COOKING_LEADING_CHARS) {
                    hits += i to GuessedIngredient(seg, inLibrary = false)
                }
                i = j
            } else {
                i++
