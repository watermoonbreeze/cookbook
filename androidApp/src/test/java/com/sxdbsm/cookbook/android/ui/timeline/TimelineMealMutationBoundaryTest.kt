...
class TimelineMealMutationBoundaryTest {
    @Test fun publicUndoFlowCallsPortOnceAndDoesNotExposeEmptyUndo() = runBlocking {
        val driver = JdbcSqliteDriver(JdbcSqliteDriver.IN_MEMORY)
        CookbookDatabase.Schema.create(driver)
        Dispatchers.setMain(UnconfinedTestDispatcher())
        try {
            val db = CookbookDatabase(driver)
            val port = CountingPort(null)
            val repository = MealRecordRepository(db)
            val vm = TimelineViewModel(MealRecordUseCase(repository), MealProjectionRepository(repository), port)
            var shown = 0
            vm.deleteDayUndoable(LocalDate(2026, 8, 26)) { shown++ }
            assertEquals(1, port.undoCalls)
    ...
    @Test fun publicUndoFlowRestoresSuccessTokenOnce() = runBlocking {
        ...
        try {
            ...
            val vm = TimelineViewModel(MealRecordUseCase(repository), MealProjectionRepository(repository), port)
            var undo: (() -> Unit)? = null; var shown = 0
    ...
    @Test fun publicMutationFailuresDoNotShowUndoAndExposeTimelineError() = runBlocking {
        val driver = JdbcSqliteDriver(JdbcSqliteDriver.IN_MEMORY); CookbookDatabase.Schema.create(driver); Dispatchers.setMain(UnconfinedTestDispatcher())
        try {
            ...
            assertEquals("删除失败，请稍后重试", vm.state.value.copyError)
            vm.deleteDay(LocalDate(2026, 8, 27))
            assertEquals(1, port.deleteCalls)
        } finally { Dispatchers.resetMain(); driver.close() }
    }

    @Test fun restoreFailureKeepsSingleUndoAndExposesTimelineError() = runBlocking {
        val driver = JdbcSqliteDriver(JdbcSqliteDriver.IN_MEMORY); CookbookDatabase.Schema.create(driver); Dispatchers.setMain(UnconfinedTestDispatcher())
        try {
            val repository = MealRecordRepository(CookbookDatabase(driver))
            val port = CountingPort(MealDayUndoToken(Any()), restoreError = IllegalStateException("restore failed"))
            val vm = TimelineViewModel(MealRecordUseCase(repository), MealProjectionRepository(repository), port)
            var shown = 0; var undo: (() -> Unit)? = null
            vm.deleteDayUndoable(LocalDate(2026, 8, 26)) { shown++; undo = it }
            undo!!.invoke()
            assertEquals(1, shown); assertEquals(1, port.restoreCalls)
            assertTrue(vm.state.value.copyError!!.contains("restore failed"))
        } finally { Dispatchers.resetMain(); driver.close() }
    }
