...
- 🧪 **I8 健康建议代际隔离（R2-A，`abbccd00`）**：确认页健康建议改为独立 `healthAdviceJob`，在编辑、改日期、重置、保存和关闭时取消并清空；回写同时校验 generation、同一 preview 对象和预览阶段，迟到成功/失败均不可污染新会话。`T-ADV-01~05`、既有同意门禁、Android 全量单测和 Debug 构建已通过；仍待旗舰 REVIEW 与 R10 真机项，未宣称运行时通过。
