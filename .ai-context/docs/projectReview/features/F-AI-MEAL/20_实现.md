...
- 🧪 **I8 健康建议代际隔离（R2-A，`abbccd00` + `5731d3ac`）**：确认页健康建议改为独立 `healthAdviceJob`，在编辑、改日期、重置、保存和关闭时取消并清空；回写同时校验 generation、同一 preview 对象和预览阶段，迟到成功/失败均不可污染新会话。审查补强：申请建议也必须仍处于预览阶段，避免错误态残留 preview 时留下无法确认的同意弹窗。`T-ADV-01~05`、既有同意门禁、Android 全量单测和 Debug 构建已通过；仍待旗舰 REVIEW 与 R10 真机项，未宣称运行时通过。
...
---
2026-08-27 回写：统一添加入口已将 AI 单天/周期两个入口收敛至 `UnifiedAddMealScreen` 的既有 `AiMealInputViewModel`；不新增第二份 AI 草稿状态。证据：`6f2b201c`（UEN 实现）与 `cd782504`（状态恢复回归）；运行时仍只认真机清单待验证项。
