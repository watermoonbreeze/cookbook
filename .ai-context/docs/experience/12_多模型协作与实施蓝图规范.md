...
## 0. 维护模式（2026-08-18 起，见 `projectReview/08` D-21）

本系统的**设计阶段已于 2026-08-18 收尾**：任务规范、门禁、失败归因（AT-01~10）、升级判据、模型画像、版本冻结与防漂移机制均已成型。此后进入**反应式维护**——只在真实批次撞到具体摩擦时才动它，不再主动去找"更先进的方案"来完善它，也不再开横向对比/架构评估类支线（D-19 护栏③的直接延续）。

允许改这套系统的情形只有四类（`EXEC/SURFACE` 满足 AT-07 时提 Level、`OMISSION`/`SCOPE` 只增强脚本机械核对、D-20 退出条件到期、使用规模发生质变需重走决策记录），细节一律看 D-21，本节不重复展开。**除此之外的"完善治理机制"想法，先写进 D-21 说的触发条件里等真实证据，不要直接开工。**
...
### 10.1 审核增量台账（2026-08-27 起强制）

- 审核任务一启动，REVIEW 在 `.ai-context/docs/arch_evidence/<批次>_REVIEW_LEDGER.md` 新建或续写台账；不得等最终结论才写。
- 每完成一个检查点立即追加：`时间 / reviewer / commit范围 / 检查ID或文件定位 / 可见证据 / 结论(PASS|AF|待查) / 下一步`。只追加，不覆盖既有记录。
- 发现 AF 时当场写入 AF 编号、违反 ID、唯一最小修复、必增测试、禁扩范围；审核中断后的接手者从最后一条 `待查` 或 AF 开始，已标 PASS 的检查点不得重复扫描，除非 commit 范围变化。
- 最终“通过/阻断/缺证据”仍须写回蓝图与 `BLUEPRINT_STATE.md`；增量台账是过程证据，不得冒充 ACCEPTED 或真机 PASS。
...
目标 / 非目标 / allowlist 固定块（见下方语法，GC-02） / 上一批延后项归宿（GC-03，逐项转本批 ID 或显式弃置）
...
### 11.1 allowlist 固定块（GC-02 的机器可解析载体，2026-08-18 起新蓝图必用）

**为什么改**：`blueprint_check.py` 原先用启发式正则从自由格式的 Markdown 表格＋粗体标记里猜 allowlist，脚本自己在文件头写明"解析是启发式的，不代表一定违规"——而它正是 AT-03 `SCOPE` 越界型偏差的唯一对症对策（AT-03 明写"提 Level 无效，对症对策是差集机械核对"）。核对工具本身会误报，对策就打了折。固定块把 allowlist 从"文档表述"变成"机器可读声明"，让 `[UNLISTED]` 结果可直接采信。

**语法**（`allowlist` 围栏块，全文只取第一个）：

````text
```allowlist
# `#` 开头为注释行；空行忽略
# `allow:` / `forbidden:` 各自独占一行分节，块开头缺省为 allow 节
allow:
<路径glob> | <允许操作说明>
forbidden:
<路径glob> | <禁改理由>
...
- **路径在 `|` 左侧**，可加反引号；`|` 右侧是给人看的自由文本，不参与匹配，写多长都行。
- 路径里的 `...` 与 `*` 都按"任意字符"展开（如 `shared/.../ai/meallog/*` 匹配该目录全部文件）。
- **一行一条**，禁止把多个路径塞进同一行——脚本按行切分。
- 不合语法的行（缺 `|`、`|` 左侧为空）**不会被静默丢弃**，脚本打印 `[WARN]` 并忽略该行；见到 WARN 必须回去修蓝图，否则等于放行了一条未声明路径。

**示例**（取自 K1i 蓝图 §6 的等价改写）：

````text
```allowlist
allow:
shared/.../ai/AiRuntimeConfig.kt | `SwitchableAiRuntime` 新增 `override fun stream(...)`（按 §4.4）；不改 `complete()` 既有逻辑
shared/src/androidUnitTest/kotlin/com/sxdbsm/cookbook/ai/SwitchableAiRuntimeStreamTest.kt | 新建，承载 T-K1I-01~04
forbidden:
androidApp/.../ai/CloudAiRuntime.kt | 其 `stream()` 真实实现已正确，本批只让它变得可达，不改它本身
shared/.../ai/meallog/* | AI 记一餐主线，本批预期零改动即受益，不主动碰
androidApp/.../ui/ai/* | 本批不涉及任何 UI
```
````

**与既有蓝图的关系（不回改历史文件）**：`blueprint_check.py` 优先找固定块，找不到才退回启发式，并在输出里打印当前走的是哪条路径。已交付的 7 份旧蓝图（B4/K1a/K1b/K1e/K1i/L1 等）**不重写**，继续走启发式分支，解析结果与本次改造前逐字节一致（已实跑比对）。表格形式的 §6 可以保留给人阅读，固定块是给机器读的那一份，两者并存时以固定块为准。
...
| 2026-08-17 | UBF 支线退休回顾 + K1i/Pro-R1 归因错判风险（§4"复发"分支析取未拆分） | — | 归因机制改造：`blueprint_protocol.md` §4 拆出 `BP`/`EXEC`/`NON-BP` 前置归因层，`EXEC` 出口不进 GC/Level 体系，改写入项目「模型能力画像」表；**不新增 GC，不开 L8** | — | — | I（本节）+ 项目 `14_模型执行力评估.md` | ARCH | `UBF退休与模型执行力评估重设计.md` §4.5 |

**当前基线：L7 · 48 条 GC · 5 条已升"审查必查"（GC-03/24/27/31/32）· 1 条已升"自动检查建议"（GC-34，复发计数 3）· GC-38~48 复发计数均为 0；不新增 L8。2026-08-17 归因机制改造未新增/删除任何 GC，总数保持 48。**
...
### 14.2 归因判据（AT-01~AT-10，2026-08-17 新增）

> 背景：`blueprint_protocol.md` §4 原"复发"分支把"蓝图本该按此写但没写"与"写了但未执行"两种根本不同的情况（蓝图缺口 vs 模型未照做）合并处置，导致执行侧偏差被错误记到 GC 头上、污染复发计数（真实案例：K1i 批次改了 allowlist 外文件，若归入"GC-02 复发"就是错误归因）。本节是该分支拆分后，`EXEC`（执行侧）出口的项目专属判据，`BP`（蓝图侧）出口仍按 `blueprint_protocol.md` §4 三分支处置。归因单位是**单个偏差项**，不是批次；聚合指标（首次通过率/AF数/返工轮数）**不得**单独作为归因依据，只用于判定是否需要启动逐项归因（AT-10）。

**AT-01（BP / EXEC 唯一分界）** — 偏差 d 的复核记录中，存在一行 `蓝图定位 = <文件>:<§>:<带ID条款>` 且存在该条款原文引用（可 grep 到），则记 `EXEC`；否则记 `BP`。定位串缺失时默认判 `BP`（故意把默认值设向"改蓝图"，防止 ARCH 把自己的疏漏推给模型）。散文段落未编号不算已闭合，归 `BP` 侧走扩容。

**AT-02（`OMISSION` 遗漏型）** — d 同时满足：①对应条款是带编号的枚举项（`STEP-`/`T-`）；②该项在 diff 中零改动；③交付台账对该项标记"完成/合规" → 记 `EXEC/OMISSION`。对症对策是外部核验/脚本机械核对，**提 Level 无效**（把同一条款写得更细不会让一个选择跳过它的模型不跳过）。实证：Pro R1「T-B5-01~03 等全部未写但台账照勾」。

**AT-03（`SCOPE` 越界型）** — 存在被修改文件 f 使 `f ∉ 蓝图 allowlist` → 记 `EXEC/SCOPE`。机械判据：`blueprint_check.py --allowlist` 对 `git diff --name-only` 与 allowlist 表求差集，非空即命中。**提 Level 无效**，对症对策是差集机械核对。实证：K1i 改 `shared/.../ai/CloudAiConsent.kt`。

**AT-04（`SURFACE` 表面合规型）** — 存在带 ID 条款 c，实现字面满足 c，且存在一个真实可达的输入/状态使 c 的意图不成立（须写出该输入/状态） → 记 `EXEC/SURFACE`。**这是唯一可触发提 Level 的执行侧类别**。实证：`AF-B456-05` 二次复核（`segmentStatuses: List<StreamSegmentState>` 字面满足 GC-17，三值枚举表达不了"尚未开始"）。

**AT-05（`NON-BP` 非蓝图问题）** — 存在可复现的外部证据串（可重跑命令 / 可引用的用户原话 / ARCH 自身 payload 缺陷定位）→ 记 `NON-BP`，不计入 GC 复发计数或模型能力记录。ARCH 自己蓝图写错导致的偏差必须走这里，不得算模型负样本。给不出证据串 → 一律降级为 `BP`，防止本类别变成自我豁免出口；`NON-BP` 归因建议每 10 批抽查一次，抽到给不出证据串的补记为 `BP`。

**AT-06（`INTEGRITY` 诚实性，正交叠加）** — 交付台账中存在被标"完成/通过/合规"的条目，其 Evidence 引用的 T-ID/文件/命令输出经 grep 证实不存在或与实况不符 → 叠加 `INTEGRITY` 标记（不替代 AT-02~04 的分类）。机械判据：`blueprint_check.py --evidence`。**容错为零，单次即入模型能力画像**，因为它破坏的是整个复核体系赖以运作的前提。

**AT-07（提 Level 的必要条件）** — 允许把某 `(模型, 任务族)` 的最低 Level 从 `Lk` 提到 `Lk+1`，当且仅当存在 **≥2 条独立记录、归因均为 `EXEC/SURFACE`、且发生在同一任务族**。仅有 `OMISSION` 或 `SCOPE` 记录不构成依据。

**AT-08（加严止损）** — 某 `(模型, 任务族)` 已达 `最低Level=项目基线上限` 且 `执行模式=ASSISTED`，仍出现同类偏差第 3 次 → 停止继续加严，裁决"不适配"。

**AT-09（撤销通道）** — 某加严项连续 2 个批次未被触发 → 标"候选撤销"，下一批 ARCH 须显式裁决保留（写理由）或撤销。只增不减的清单是税，不是治理。

**AT-10（聚合指标的唯一合法用途）** — `AF数≥1` 或 `返工轮数≥1` 或 `越界数≥1` → 必须对每个偏差逐条填 AT-01~AT-06；全为 0 → 整批记"无偏差"，跳过归因，画像表登记正样本。

具体记录格式、任务族枚举、模型能力画像表见 `docs/experience/14_模型执行力评估.md`；机械检查脚本见 `.ai-context/tools/blueprint_check.py`。模型名称、行为观察和跨模型能力数据不在本规范中复制。
...
> **出身说明（2026-08-18 独立裁决订正）**：§14.6~14.8 是**本项目原创**的治理规则，不是与用户级 `blueprint_protocol.md` 逐条对齐/镜像的产物——`GOV-BP-P3-01` 审计原稿曾把三节包装成"与全局 canonical 条款对齐"，经独立复核确认该对齐关系不成立（全局文件里查无对应条款），已在 `GOV_BP_P3_01_AUDIT.md` 就地作废。三节规则本身内容自足、已生效、不受此订正影响，只是不再声称"源自全局"。详见 `08_决策记录.md` D-22。
