...
4. **`.ai-context/docs/feature/待办总览.md`** —— 全量待办（按 session 类型分区 + 顶部 2026-07-26 战略会诊三张单）。
...
- ⏳ **下一步候选**（各单开 session·过门禁）：成员化健康红绿灯（P0 头号）、引擎正向兑现到推荐质量（多成员折算贯穿推荐 + 忌口扩正向"推有利菜"）、生命阶段调养做透一个场景（P1）、餐食系数改占比%呈现、条码扫描、基础顺手度打磨。详见 `待办总览.md` 顶部 2026-07-26 块。
...
## 八 待办与下一步
- 全量：`feature/待办总览.md`（分区：数据/健康·参考资料驱动·功能算法·UX·工程·真机 bug 等）。
