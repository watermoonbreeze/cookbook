...
### 9.42 权威份量的去处：参考页详列 · 推荐动线只给轻入口 · 绝不塞克数（P3 份量对齐呈现）
> [AI生成 2026-07-25] P3"推荐搭配份量对齐膳食宝塔"呈现层·apple_ux_designer 会诊 + 算法会诊 + 用户拍板"呈现补齐+口径收敛"·Google 质量终审无阻断。核心裁决：**权威每日克数份量只在参考页详列，高频推荐动线绝不铺克数/进度条/达成度**（数据不可靠+份量焦虑+免责红线）。
- **份量三层归位**：①**膳食参考依据页**(我的·下)=权威每日份量(`DAILY_AMOUNTS` 谷薯200-300g…)唯一详列处·带口径出处·**已达标无需改**；②**推荐结果**(单餐+周期)只显该餐次**定性结构建议**(§9.41 的 hint·不出克数)；③**今日卡**仅命中偏离补一条(§9.41)。克数**不下放**到②③。
- **①覆盖所有推荐入口(统一范式)**：P2 只在周期入口(`AiPlanScreen`)显餐次结构建议行，**P3 补到单餐入口**(库存/随机推荐 `AiRecommendScreen`·`MealStructureHintRow`)——同一 `DietaryGuideline.hint` 来源/同灰字样式。**单餐场景**同一次推荐多套方案共用同一餐次→**结构建议提到结果区顶部只出一次**(非逐方案卡重复·避免刷同句噪声)；**周期场景**每餐名下逐餐一行(自然阅读节奏)。选"全部"无餐次→`mealKindOf` null 不显(沿用降级)。
- **②权威份量轻入口**：推荐页/周期计划页脚加**文字型**"各类每天吃多少 ›"(`DailyAmountRefLink`·`TextButton` 无卡片无图标)→跳膳食参考依据页。让权威份量**可达可溯源**却不喧宾(吸收"静态份量参考"·不孤立贴份量表)。四入口都放页脚、各出一次。
