...
> 状态：`ACCEPTED` —— 2026-08-07 架构模型复核（覆盖 B4+B5+B6）未通过，9 项阻断 `AF-B456-01~09`；二次复核 8 项已关闭，`AF-B456-05`（segmentStatuses 值域覆盖不全）第二轮已关闭（Coder@副机·deepseek-v4-pro）。**三次复核（ARCH@主力机）通过**：3 处代码改动与 §3.5.1 逐字核对一致，`GenerationProgressTest` 4 条测试真实存在且实跑 4/4 绿（非台账自报）；发现 `T-B5-02` 断言弱于蓝图要求（用"任一终态"而非精确 `listOf(FAILED, COMPLETED)`，无法捕获"FAILED 信号被吞、两段同判 COMPLETED"这类回归）并现场收紧为精确匹配，收紧后复跑仍 4/4 绿，`androidApp:testDebugUnitTest` 全量无新增失败。AF-B456-05 正式关闭，本蓝图批次全部 9 项阻断关闭完毕。
...
| GC-17 | 列表逐项状态由数据层产出 `List<Status>` | §3.5 + §3.5.1（2026-08-07 二次复核追加"未开始"修订，第二轮已关闭） | ✅ 满足（`segmentStatuses` 类型改 `List<StreamSegmentState?>`，null="未开始"，`SegmentProgressBar` 补 `null->PENDING` 分支，T-B5-01~04 4/4 绿） |
...
| GC-22 | 可见副作用配 T-ID/真机项 | §9.4 + §13 | ✅ 满足（T-B5-01~04 已新增 4/4 绿，真机项已登记，§9.4"当次结果"列已补填） |
| GC-23 | 实施脚本最小动作独立编号 STEP-ID | §7 全节（改写为 STEP 编号） | 满足（蓝图文本已具备，属 ARCH 产出，非 CODE 待办） |
| GC-24 | 交付台账 STEP 勾销表 | §7 步骤 6（新增） | ✅ 满足（Evidence 列已补填真实 commit，T-B5-01~04 4/4 绿，不再引用不存在测试 ID） |
...
| INV | T-ID | 覆盖 AF | 当次结果 |
|---|---|---|:--:|
| INV-B4-01~11（原有） | T-B4-01~07（§9.2） | — | ✅ T-B4-01~07 全部通过（第一轮） |
| （字段真相源 §7 步骤 2.0） | T-B4-08/09/10（字段真相源） | AF-B456-01/02 | ✅ 二次复核逐点核对 W1~W6，`grep "inputText ="` 赋值形零命中 |
| INV-B456-R05a/b/c/d（§3.5 + §3.5.1） | T-B5-01/02/03/04 | AF-B456-05 | ✅ 第二轮关闭：`segmentStatuses: List<StreamSegmentState?>` + null→PENDING + 去掉兜底，GenerationProgressTest 4/4 绿 |
| INV-B4-05/06（截断提示，§3.6） | T-B4-05a/05b（沿用）+ 真机 `E-B6-TRUNC-01` | AF-B456-04 | ✅ 二次复核核实截断 Snackbar 调用点存在 |
| segmentId 唯一性（§3.4） | `StreamingMealSessionTest` 新增用例 | 缺证据 §3.3 | ⚠️ 低优非阻断（`InputSegmentFactory` 保证唯一，Session fail-fast 已到位） |
| §5.8 语音生命周期 | 无法单测，真机 `E-B6-VOICE-01` | AF-B456-03 | ✅ 二次复核核实 `activeRecognizer` 已改 `MutableState` 传递 |
| §7 步骤 3.4 标题文案 | 无单测，并入真机 `E-B4-01` 预期结果 | AF-B456-09 | ✅ grep `"AI 周期记一餐"` 命中 |
