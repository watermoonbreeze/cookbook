...
| INV-L1-12（v2b 新增，回应 §10 v2 挑战第7项；**v2c 勘误**——真机 E-L1-03 实测暴露 v2b 原文本身有缺陷，已修订，见下） | `state.type==CLOUD && state.cloudAiConsent.status==DECLINED`（用户拒绝/关闭后又手动把单选切回"云端大模型"，`onTypeChange` 本身无校验、本批不改它） | `CloudSection` 渲染一行"云端 AI 已被你关闭 · 重新启用 ›"，点击**必须先**用 `reenableKeyDraft(state.keyByVendor, vendor)`（见 hotfix 蓝图 `L1_云端AI首启同意_真机反馈修复_实施蓝图.md` §3 STEP-1/§8.1 STEP-4）**填充 `pendingKeyDraft`**，若返回空串（Key 已被用户删除）则改为打开 `KeyDialog` 引导先补填，非空才 `consentPanelOpen=true`（复用完整同意面板，走与首次同意相同的 `grantConsent`/`declineConsent` 路径，不新增第三种同意函数） | 该状态下用户唯一的恢复路径是"打开某个厂商的 KeyDialog 重新按一次保存"（不可发现，见挑战原文）；`CloudSection` 因 `status!=GRANTED` 完全不渲染任何相关提示（用户会以为"选了云端却什么都没有"，找不到出路）；**v2c 新增禁止项**：点击直接 `consentPanelOpen=true` 而不填充 `pendingKeyDraft`（真机 E-L1-03 实测：`grantConsent` 会用空串覆盖已保存的 Key，把用户"重新启用"的操作变成"清空密钥"）；以空 Key 直接进 `grantConsent` 而不判空转向 `KeyDialog`（会导致"已启用"提示与"密钥未配置"同时成立，违反透明准则） | T-L1-12，真机 E-L1-03（已修复，见 hotfix 蓝图） |
...
**真机待验证登记**：`E-L1-01`~`E-L1-12` 已登记至时间戳最新的 `真机待验证清单_202608082330.md`（覆盖 grandfather 补确认/常驻状态块/关闭重启用/首同意流程/暂不启用/换厂商确认/未同意不外发/政策披露/同意时间/删密钥/冷启动互斥/全新安装共 12 项，含 apple_software_behavior 行为契约对应的可验证场景）。
