...
| **TURN** | **CODE**——蓝图就绪，交由 `luna`（远程主机编码 agent）执行，本机 ARCH 停手等待交付 |
...
| 基线 sha | 待 CODE 交付首个 commit 时回填（本批次尚未产生代码 commit） |
| 交付 commit | 待回填 |
| 全景图回写 | 待回填（`BLUEPRINT_READY`状态不要求本字段，仅 `ACCEPTED` 时必填，见文件头说明） |
| 下一步 | ①`luna` 读蓝图 §0 列的5份材料，按 §20 Phase 1-5 顺序执行，每个 STEP 按 §21.2 模板勾销；②交付前跑 `google_quality_engineer` 复审（蓝图 §24 第6条）；③交付时更新真机待验证清单 `E-UEN-01~15`（蓝图 §21.3）；④交付后本机 ARCH 复核（diff 逐文件核对 + 实跑三条构建命令 + INV 逐条对照），通过后回写本表状态→`ACCEPTED`+基线sha+交付commit+全景图回写，`TURN`→`USER`。 |
