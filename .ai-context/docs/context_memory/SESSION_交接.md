...
## 功能完成交接（2026-08-29 · HOME-MERGE）

- `HOME-MERGE-01` 已由事实复核转为 **ARCH_ACCEPTED / AUTOMATED_GATES_PASS / PENDING_DEVICE_VERIFICATION**；详情见 `2026-08-29_HOME-MERGE_功能完成交接.md`。
- 数据边界测试与 Android Debug 构建通过；首页“今天和接下来”顺序、占位卡、真实未来卡上限、常驻食历入口均已落地。
- 下一会话不得重复开发 HOME-MERGE；仅在 R10 真机回归执行 `E-HM-01~08`，涉及视觉/无障碍/导航的结论不得从 JVM 测试推断。

## 功能完成交接（2026-08-29 · Bug-5305 / P5-A）

- Bug-5305 与 P5-A 均已完成事实复核和独立交接，分别见 `2026-08-29_Bug-5305_功能完成交接.md`、`2026-08-29_P5-A_功能完成交接.md`。
- 二者的当前定向回归与 Android Debug 构建均通过；运行时分别只剩 `DEV-5305-01` 和 `DEV-P5-01~05`，统一由 R10 真机阶段处理。
- 下一会话不得重复实现两批；P5-B 及 Phase 6 仍未授权。
...
- R4 Reality 已登记至 `HEALTH-NUTRITION-CONTRACT-01_研究与决策蓝图.md`：营养当前实时计算，阈值与人工 care 规则需整体审计；已定位 WHO/国家卫健委证据入口但不据搜索直接改数值。历史快照、调味料、多成员、生命阶段均单列 ADR/实施批。
- R5 Reality：推荐已有硬安全集、多风格权重和共享周营养线，但营养线基于主料名称且尚不等于完整营养计划。`RECOMMEND-PLAN-01_研究与决策蓝图.md` 固定先做可复现排序，再做健康范围、周计划事实语义、显式偏好和标签；禁止隐式学习/自动写入。
- R6 Reality：当前 `eaten_ratio` 只表达摄入比例，计划/历史共用餐记录，库存又将未来计划纳入派生占用；无统一计划/已吃/跳过/外食/成员份额状态。`MEAL-SEMANTICS-01_研究与决策蓝图.md` 已要求先 ADR、迁移与幂等合同，禁止日期自动结算。
- R7 Reality：交互待办已按无障碍基础、我的页、食历、食材发现、AI 全屏入口、导航胶囊和 Material3 兼容实验分为 R7-A~G；旧 Sheet 的系统长按问题不能靠删除粘贴兜底解决。详见 `UX-CONSISTENCY-01_研究与蓝图目录.md`。


## 下一会话启动卡（2026-08-27 · L2 脂肪肝 App 入口）

- L2 已完成独立蓝图握手、自动化验证与独立架构复审，状态为 `ARCH_ACCEPTED / AUTOMATED_GATES_PASS`、`TURN=NONE`。
- 实现复用 `care_fatty_liver` 的既有 category-id → 动态家庭档案 → `member_care` → 规则消费者链路；仅新增参考页食材级透明说明与三组回归测试。未改 seed、schema、健康枚举、推荐算法或 Family UI。
- 复审 AF-L2-01~03 均已关闭：规则原始数量/reseed 完整性、成员→全家并集、逐条来源口径均有覆盖。参考页只作“食材级参考”，不暗示所有规则由单一指南逐条背书。
- 真机项 `DEV-L2-01~04` 位于 `真机验证/真机待验证清单_202608272229.md`，均保持 `PENDING_DEVICE_VERIFICATION`。

### 后续候选（不在本批实施）

1. `F-RECOMMEND` R6 第二批“换一换均衡档”：云端预取多批 + 本地轮播；需新建独立蓝图，先复核推荐缓存、健康约束每次重取与日志覆盖，不能沿用本批 allowlist。
2. `F-AI-MEAL` K1g/K1b 的周期记与健康档案链路：已有实现和待验项，先按唯一真机清单统一验证；若发现问题再单独建缺陷批次。
3. `F-FAMILY` 多人关注完整重构：产品方向已确认但仍缺 Apple UX 布局细则，属于待决项，不得直接编码。
4. 统一真机/OEM 验证：包括 L2 `DEV-L2-01~04`、Bug-2119、Bug-5305、P5-A、可观测性等，按用户“最后统一执行”授权排在全部代码批次之后。
