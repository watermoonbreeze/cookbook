...
> **接手推进**：读 `待办总览`(唯一 backlog 真相源) + `健康判定_数值加属性双层.md`/`食材属性标签体系设计.md`/`自建食材双层判定_方案讨论.md`(六=UI交互规范) + 本档 → **数据体系大轮全收尾**(数据扩充/交叉核对/资产化/双层标准/属性标签体系/加工食品/食材数据全处理·经验已总结)。**⏭ 明确下一步 = 自建食材 L3 UI 实现**(shared逻辑`FoodAttribute`/`AttributeGuesser`+单测已 push·Apple-UX 交互规范已出·据规范实现 `IngredientEditorDialogs.kt` 属性勾选区+`AttributeGuessBanner`提示确认+submit展开care·守 hydrated/Saver 复杂表单红线·**用户已定单独一轮谨慎做**)。其余需用户介入：#203食材编辑器路由化(在场)、A#6-B暗因子(拍板)、旧 #209/#210 健康忌口补漏(用户亲核)、#208更新弹窗、#177运营、营养表体现属性+冻结左列(食材体系落地后)、数据性能专项规划(数据源增多)。每功能过门禁+构建+push·真机验证用户统一做。
