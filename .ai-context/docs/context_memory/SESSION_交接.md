...
> 当前状态：AI 记一餐 V2 追加验证通过；周期记 + NDJSON 流式大改的 B1/B2 五审仍不通过，须先完成 AF-15 至 AF-18；B3 被阻断。
...
> 2026-08-05 架构基线已补齐：实施必须先读 `feature/AI记一餐_周期记_NDJSON流式开发规范.md`，按 B1 至 B6 分批交付；当前无老版本，禁止旧协议兼容、数据迁移与双轨状态。`35a18c8e` 已采纳 §7.5 的主抽象，但五审发现 AF-15~18；唯一修复依据为落地方案 §7.6，固定处理取消、IO 重试、真实测试夹具和回归测试恢复，未关闭前不得开始 B3。
