...
4. ✅ **菜品页餐次升为一级Tab**（待提交·构建绿+审查中）：**用户澄清=就是把"餐次"加成和 最近/喜爱/菜系/家庭 同排的一级Tab**(不是方案A的导航重构·我之前想复杂了·用户明确要最简做法)。实现:`DishesSortTab` 加 `SLOT`;sortTabs=最近/喜爱/菜系/餐次/家庭;选中"餐次"Tab 像菜系Tab那样**左侧竖栏列餐次分堆**(复用 CuisineRail·全部/早餐/中餐/加餐/晚餐/宵夜·label↔DishSlotFilter映射)+右侧对应菜(拼音分组+字母索引);餐次筛门控到SLOT档(同菜系筛只在菜系档);**移除原全局二级餐次横条**。落 `DishesViewModel`/`DishesScreen`。**经验**:用户给明确指令时直接做最简实现·别过度设计成大重构(方案A文档保留作参考·未采用)。
+ **附带**：移除记一餐选菜页"最近"档下重复的"最近常吃/喜爱"区(`0e3801c`·与主Tab重复)。
