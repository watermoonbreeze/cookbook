...
- 未用 import 已清理（脚本按简单名启发式删 142 个，保留通配 .*，构建通过）。

## 待确认队列（高风险，无人值守未擅自做）

- `selectionMode` 布尔散落分叉 → 改为参数/回调区分管理/选择模式（深度重构，行为敏感，建议在线做）。
- 属性/维度区通用组件 DimensionSection（偏新功能，非纯重构）。
- 详情四区顺序按文档重排（做法>忌口>属性>关联菜品）——打磨项。
