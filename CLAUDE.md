...
**每完成一个功能或 bug 修复，必须更新 `.ai-context/docs/真机验证/` 下的真机验证清单。清单拆为两份（2026-08-29 用户确立）**：
- `真机验证清单-待验证_<yyyyMMddHHmm>.md` —— **唯一活跃待验证文件**（用户只看这份）。新增验证项时更新原文件并改名为当次时间戳，禁止复制/新建第二份。
- `真机验证清单-通过归档.md` —— **累计归档文件**，按日期分类，供查阅。

每项登记含：
- **编号** + 一句话标题
- **分步验证操作**（①②③…每一步用户可在真机上执行，写出具体操作路径和预期结果）
- **判据必须是真机能看到的现象，或日志里能查到的内容（2026-08-19 用户确立）**：预期结果里写的每一条判据，必须是①App 界面上用户操作后能亲眼看到的东西（文案/图标/卡片/动画），或②`adb logcat`/`AppLogger` 导出文件里能搜到的字样；**不能写"诊断信息里 Delta 事件次数"这类 App 里根本没有暴露、用户无从查证的内部计数**（K1i 的 E-K1I-01 就踩过：判据要求数 Delta 次数，但代码里这个数字从未打印到日志或界面，用户想验也无从下手）。写判据前先确认这个信号真的可见/可查（不确定就去读代码确认有没有打印/展示），拿不准就换成"确认页菜品卡片是否逐条冒出来"这类肉眼能判的替代信号，而不是留一条验证不了的判据卡在清单里。
- **状态标记**：🔧=已修复待真机确认 / ⬜=待验证 / ✅=已验证通过 / 🚫=需求变更·范围收窄（不再需真机）
...
**迁移规则（2026-08-29 用户确立·强制）**：用户在"验证结果"列填"通过"后，AI 必须把该行从「待验证」文件**迁移**到「通过归档」文件「✅ 已通过」分节的对应日期分组（按验证日期归类，无则新建日期小节），并从「待验证」文件移除该行；`🚫`/`过期`/`需求过期` 等已关闭项同样迁入归档文件的「🚫 已关闭·需求变更/范围收窄」分节。**待验证文件只保留尚未验证（验证结果列为空）的项**，让用户只关心这一份即可。迁移后核对行数无丢失。
