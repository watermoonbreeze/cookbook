...
**App 的所有行为都必须让用户知道——不能有静默行为。** 凡 App 在用户看不见处**自动做的事**（后台自动改数据、自动重跑 seed / 迁移 / 纠正存量、联网拉取、权限使用、缓存清理、自动同步/备份、埋点采集等），都必须有**明确的用户可见反馈**（弹框/横幅/通知/Snackbar/设置项状态），并**写清"做了什么 + 为什么 + 影响什么"**（数据变更要有 changelog / what's-new，不能只说"已更新"）。守苹果"诚实、克制、不操纵、用户掌控"——告知是尊重不是营销，禁暗黑模式（默认勾选/诱导/制造焦虑/假紧迫）。
